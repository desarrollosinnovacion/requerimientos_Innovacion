...
export function PasswordGenerada({ correo, password }: { correo: string; password: string }) {
  ...
  return (
    <div role="status" className="rounded-md border border-brand-200 bg-brand-50 p-4 text-sm text-brand-900">
      <p className="font-medium">Contraseña temporal para {correo}</p>
      <div className="mt-2 flex flex-wrap items-center gap-2">
        <code className="select-all rounded bg-white px-3 py-1.5 font-mono text-base tracking-wide text-slate-900 ring-1 ring-brand-200">
          {password}
        </code>
