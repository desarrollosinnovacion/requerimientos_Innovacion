 ...
 * Con `compacto` se queda en solo icono también en grandes (menú oculto).
 */
export function NavLinks({ enlaces, compacto = false }: { enlaces: Enlace[]; compacto?: boolean }) {
      ...
      {enlaces.map((e) => {
        ...
        return (
          <Link
            ...
            className={`flex items-center justify-center gap-3 border-l-2 px-3 py-2.5 text-sm font-medium transition ${compacto ? "" : "lg:justify-start"} ${
              activo
          ...
          >
            ...
            <span className={compacto ? "hidden" : "hidden lg:inline"}>{e.etiqueta}</span>
          </Link>
        );
      })}
    </nav>
